...
class L_SSBroyden(Optimizer):
    ...
    def __init__(
        ...
        *,
        ...
        c2: float = 0.9,
        max_ls: int = 20,
        tolerance_grad: float = 1e-30,
        tolerance_change: float = 1e-30,
    ):
        if lr <= 0:
    ...
    def _compute_Hy_recursive(
        self, y, history_s, history_y, history_tau, history_phi, history_rho, idx, gamma
    ):
        ...
        H_y_i = self._compute_Hy_recursive(
            y_i,
            history_s,
            history_y,
            history_tau,
    ...
    def _two_loop_recursion(
        self, g, history_s, history_y, history_tau, history_phi, history_rho, gamma
    ):
    ...
    def _compute_tau_phi(
        ...
        if torch.abs(rho_k_inv) < 1e-10:
            return torch.tensor(1.0, device=y_k.device), torch.tensor(
                1.0, device=y_k.device
            )  # Skip update if y·s is too small
        ...
        rho_k_minus = torch.minimum(
            torch.tensor(1.0, device=y_k.device),
            h_k * (1 - torch.sqrt(torch.abs(a_k) / (1 + a_k))),
        )
        theta_k_minus = (rho_k_minus - 1) / a_k
        theta_k_plus = 1 / rho_k_minus
        theta_k = torch.maximum(
            theta_k_minus, torch.minimum(theta_k_plus, (1 - b_k) / b_k)
        )

        rho_k_cap = (
            torch.minimum(torch.tensor(1.0, device=y_k.device), 1 / b_k)
            if b_k > 0
            else torch.tensor(1.0, device=y_k.device)
        )
        ...
        tau_k = torch.maximum(
            torch.tensor(1e-8, device=y_k.device),
    ...
    def _line_search_strong_wolfe(
        self, closure, x_k, f_k, g_k, p_k, g_dot_p, c1, c2, max_ls
    ):
        ...
        for _ in range(max_ls):
            ...
            # Armijo condition
            if f_new > f_k + c1 * alpha * g_dot_p or (
