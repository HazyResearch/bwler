...
# From https://github.com/lululxvi/deepxde/blob/master/deepxde/optimizers/pytorch/nncg.py
...
class NysNewtonCG(Optimizer):
    ...
    def step(self, closure):
        ...
        g_tuple = [g.contiguous() for g in g_tuple]
        g = torch.cat([gi.view(-1) for gi in g_tuple if gi is not None])

        # Update preconditioner every update_freq iterations (this is also a time-consuming step, but not every iteration)
        if self.n_iters % self.update_freq == 0:
            self.update_preconditioner(g)
        ...
        for group_idx, group in enumerate(self.param_groups):
            ...
            print(
                f"Time taken to compute Newton direction: {time() - start_time} seconds"
            )
    ...
    def update_preconditioner(self, grad):
